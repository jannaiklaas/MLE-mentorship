...
def objective(trial, X_train, y_train, X_val, y_val):
    with mlflow.start_run(nested=True):  # Use nested=True to create a nested run
        params = {
...
def plot_shap_values(model, X, feature_names):
    ...
    plt.close()
    mlflow.log_artifact(shap_plot_path, conf['exp_1']['artifact_path']+\
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.close()
    mlflow.log_artifact(confusion_matrix_path, conf['exp_1']['artifact_path']+\
                        conf['general']['figure_dir'])
    shutil.rmtree(temp_dir)
...
def main():
    ...
    with mlflow.start_run():
        ...
        model = LogisticRegression(**study.best_params).fit(X_train, y_train)
        ...
        tags={
            ...
            "model_architecture": conf['exp_1']['model_type'],
            "feature_set_version": conf['exp_1']['feature_set_version'],
        }
        )
