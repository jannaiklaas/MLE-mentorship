...
TRAIN_FILE_PATH =  os.path.join(ROOT_DIR, conf['exp_5']['script_file_path'])
...
def plot_shap_values(model, X, feature_names):
    ...
    plt.close()
    mlflow.log_artifact(shap_plot_path, conf['exp_5']['artifact_path']+\
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.close()
    mlflow.log_artifact(confusion_matrix_path, conf['exp_5']['artifact_path']+\
                        conf['general']['figure_dir'])
    shutil.rmtree(temp_dir)
...
def main():
    ...
    with mlflow.start_run():
        ...
        X_train, y_train, X_val, y_val, feature_names = preprocess(data, 
                                                                   col_to_drop=conf['exp_5']['col_to_drop'])
        ...
        mlflow.xgboost.log_model(
            xgb_model=model,
            artifact_path=conf['exp_5']['artifact_path'],
            signature=infer_signature(X_val, model.predict(X_val)),
            registered_model_name=conf['exp_5']['model_name'],
            input_example=X_val
        )
        ...
        tags={
            ...
            "model_architecture": conf['exp_5']['model_type'],
            "feature_set_version": conf['exp_5']['feature_set_version'],
        }
        )
        mlflow.log_params(study.best_params)

        mlflow.log_artifact(os.path.join(ROOT_DIR, 'requirements.txt'), 
                            conf['exp_5']['artifact_path'])
        mlflow.log_artifact(TRAIN_FILE_PATH, conf['exp_5']['artifact_path']+\
