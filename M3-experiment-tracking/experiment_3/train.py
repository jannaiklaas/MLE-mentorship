...
def plot_shap_values(model, X, feature_names):
    ...
    plt.close()
    mlflow.log_artifact(shap_plot_path, conf['exp_3']['artifact_path']+\
...
def plot_confusion_matrix(y_true, y_pred):
    ...
    plt.close()
    mlflow.log_artifact(confusion_matrix_path, conf['exp_3']['artifact_path']+\
                        conf['general']['figure_dir'])
    shutil.rmtree(temp_dir)
...
def main():
    ...
    with mlflow.start_run():
        ...
        X_train, y_train, X_val, y_val, feature_names = preprocess(data, 
                                                                   col_to_drop=conf['exp_3']['col_to_drop'])
        ...
        tags={
            ...
            "model_architecture": conf['exp_3']['model_type'],
            "feature_set_version": conf['exp_3']['feature_set_version'],
        }
        )
